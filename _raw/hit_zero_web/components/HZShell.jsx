...
const NAV_CONFIG = {
  ...
  athlete: [
    ...
    { id: 'messages', label: 'Team Feed',       icon: 'megaphone' },
  ],
  parent: [
    { group: "Kid's World" },
    ...
    { id: 'billing',  label: 'Billing',         icon: 'billing' },
    { id: 'sessions', label: 'Schedule',        icon: 'calendar' },
    { id: 'messages', label: 'Gym Feed',        icon: 'megaphone' },
  ],
};
...
function App() {
  ...
  return (
    <div className="app-shell">
      <Sidebar nav={nav} active={screenId} session={session} onNav={(id) => { location.hash = '#' + id; }} />
      <Topbar session={session} onOpenCmdk={() => setCmdkOpen(true)} onSignOut={async () => { await window.HZdb.auth.signOut(); }} snap={snap}/>
...
// ─── Sidebar ───
function Sidebar({ nav, active, session, onNav }) {
  const role = session.profile.role;
  return (
    <aside className="sidebar hz-nosel">
...
// ─── Topbar ───
function Topbar({ session, onOpenCmdk, onSignOut, snap }) {
