...
async function hydrate({failSkills=false,failBilling=false,throwTeams=false,priorSkills=[]}={}) {
 ...
 const supa={from(table) { queries.push(table); let from=0,to=Infinity; const chain=new Proxy({}, {get(_,key) {
  if(key==='then') return (resolve,reject)=> {if(throwTeams&&table==='teams')return Promise.reject(new Error('network unavailable')).then(resolve,reject);return Promise.resolve((failSkills&&table==='skills' || failBilling&&table==='billing_accounts') ? {data:null,error:{message:'fixture failure'}} : {data:(data[table]||[]).slice(from,to+1),error:null}).then(resolve,reject);};
 ...
 let error;try {await context.run();}catch(err){error=err.message;}return {raw,queries,error};
}
...
test('billing hydration failures remain errors instead of confirmed empty accounts',async()=>{
 const {raw}=await hydrate({failBilling:true});assert.equal(raw.__financialData.status,'error');assert.equal(raw.__financialData.loadedAt,null);assert.ok(raw.__financialData.failed.includes('billing_accounts'));
});
test('thrown network failures publish a retryable financial state',async()=>{
 const {raw,error}=await hydrate({throwTeams:true});assert.equal(error,'network unavailable');assert.equal(raw.__financialData.status,'error');assert.equal(raw.__financialData.loadedAt,null);
});
