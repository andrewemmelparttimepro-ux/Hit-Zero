...
export async function sendPaymentReminders(profile: any, body: any) {
  if (!profile?.program_id || !['coach', 'owner'].includes(profile.role)) return json({ error: 'Staff access required.' }, 403);
  ...
  if (regError) throw regError;
  let rows = regs || [];
  // Never expand selected attempts into sibling or unrelated balances.
  const { data: reviews, error: reviewError } = rows.length ? await supa.from('registration_reconciliation')
    .select('id,disposition').eq('program_id',profile.program_id).in('id', rows.map((r: any) => r.id)) : {data:[],error:null};
  if (reviewError) return json({ error: 'Payment reconciliation is unavailable. No reminders were sent.' }, 503);
  const eligible = new Set((reviews || []).filter((r: any) => r.disposition === 'active_failure').map((r: any) => r.id));
  const excluded = rows.filter((r: any) => !eligible.has(r.id)).map((r: any) => ({id:r.id,ok:false,skipped:true,reason:'requires_payment_review'}));
  rows = rows.filter((r: any) => eligible.has(r.id));
  ...
  const skippedResults: any[] = [...excluded];
